...
class CreateAlumnoComponent extends Component {
    ...
    saveAlumno = (e) => {
        e.preventDefault();
        let alumno = { alumnoID: this.state.alumnoID, nombreAlumno: this.state.nombreAlumno, apellidoAlumno: this.state.apellidoAlumno};
        console.log('Alumno guardado')
        console.log('tipo =>' + JSON.stringify(alumno));
        CursoService.createAlumno(alumno).then(res => {
            console.log(res.data)
        })
        window.location.href = "./reporte-alumno";
    }
